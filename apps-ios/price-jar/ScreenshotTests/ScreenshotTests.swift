...
final class ScreenshotTests: XCTestCase {
    ...
    func testCaptureMainScreens() throws {
        ...
        XCTAssertTrue(tabButton("Price Book").waitForExistence(timeout: 15))
        tabButton("Price Book").tap()
        XCTAssertTrue(app.buttons["priceBook.row.Bananas"].waitForExistence(timeout: 10))
        ...
        XCTAssertTrue(app.otherElements["history.chart"].waitForExistence(timeout: 10)
                      || app.staticTexts["history.best"].waitForExistence(timeout: 10))
        ...
        XCTAssertTrue(priceField.waitForExistence(timeout: 10))
        priceField.tap()
        priceField.typeText("0.85")
        app.buttons["record.save"].tap()
        XCTAssertTrue(app.buttons["verdict.save"].waitForExistence(timeout: 10))
        capture(named: "03-verdict")
        app.buttons["verdict.save"].tap()
        XCTAssertTrue(app.buttons["history.recordPrice"].waitForExistence(timeout: 10),
        ...
        XCTAssertTrue(app.buttons["shoppingList.addFromBook"].waitForExistence(timeout: 10))
        app.buttons["shoppingList.addFromBook"].tap()
        settle()
        let addRow = app.buttons["shoppingList.addFromBook.row.Bananas"]
        XCTAssertTrue(addRow.waitForExistence(timeout: 15))
        addRow.tap()
        XCTAssertTrue(app.staticTexts["Bananas"].waitForExistence(timeout: 10))
