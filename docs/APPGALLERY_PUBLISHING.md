...
| **Enable App Signing (method 1)** | App > Distribute > Services > App Signing | Do this **before the first package upload**, always -- Huawei hides the method-1 option the moment any package exists, and the only remedy is creating the app again. There is no API to enable it, none to read it back, and no error when it is missing: Huawei does not enforce upload-key consistency, so a misconfigured app publishes normally and the damage surfaces at the next version. AGC then generates and keeps the signature key, so a lost local keystore never bricks the app. Huawei does not allow changing a signature key afterwards ("the signature key or upload key cannot be changed after being uploaded; you need to create an app again"), and enrolling a released app requires uploading the original key — the one you would have lost. The owner already has five published apps that can never be updated because of this. |
...
| **Compatible devices** | App information > Compatibility | A new app is created with nothing ticked, and the version page then refuses to submit with a banner that only says "one or more items need to be set on the App information page". The real error appears only after pressing **Next** on that page: "Select at least one compatible device." Tick **Mobile phone**. This is not the same thing as the `deviceTypes`/`appAdapters` the API sends with `app-config`; that call succeeds while this box is empty. |
