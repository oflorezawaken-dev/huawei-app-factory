...
| **Enable App Signing (method 1)** | App > Distribute > Services > App Signing | Do this **before the first upload**, always. AGC then generates and keeps the signature key, so a lost local keystore never bricks the app. Huawei does not allow changing a signature key afterwards ("the signature key or upload key cannot be changed after being uploaded; you need to create an app again"), and enrolling a released app requires uploading the original key — the one you would have lost. The owner already has five published apps that can never be updated because of this. |
