 ...
 * alarm permission, battery friendly, good enough for plants.
 */
class DailyReminderWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {

    override suspend fun doWork(): Result {
        val container = (applicationContext as PlantCueApplication).container
        val due = runCatching { container.plantRepository.dueItems() }.getOrElse { return Result.retry() }
...
object ReminderScheduler {
    ...
    private const val UNIQUE_NAME = "daily_care_reminder"
