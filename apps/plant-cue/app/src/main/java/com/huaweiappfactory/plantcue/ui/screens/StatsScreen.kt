...
import com.huaweiappfactory.plantcue.ads.InterstitialTrigger
import com.huaweiappfactory.plantcue.ads.LocalAdManager
import com.huaweiappfactory.plantcue.ads.PetalBanner
import com.huaweiappfactory.plantcue.ads.findActivity
import com.huaweiappfactory.plantcue.ui.LocalInterstitialsSuppressed

@Composable
fun StatsScreen(viewModel: StatsViewModel) {
    val stats by viewModel.stats.collectAsState()

    // Natural pause: the user is looking at numbers, not caring for a plant.
    ...
    Column(Modifier.fillMaxSize()) {
        Column(
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                StatTile(stringResource(R.string.stats_plants), s.plantCount.toString(), Modifier.weight(1f), highlight = true)
                StatTile(stringResource(R.string.stats_streak), s.streakDays.toString(), Modifier.weight(1f))
            }
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                StatTile(stringResource(R.string.stats_events_month), s.eventsThisMonth.toString(), Modifier.weight(1f))
                StatTile(stringResource(R.string.stats_on_time), s.onTimePercentThisMonth?.let { "$it%" } ?: stringResource(R.string.stats_no_data), Modifier.weight(1f))
            }
            if (s.eventsThisMonth == 0) {
