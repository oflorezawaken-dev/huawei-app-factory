# AppGallery privacy tags checklist: Hashtags (hashtags)

Console path: **AppGallery Connect > Apps and atomic services > Hashtags > Version information > Privacy tags**

Generated from `apps/hashtags/store/privacy-tags.json`. Labels are the official English console names; a localized console translates them but keeps the same order and grouping.
...
5. Record the date in `factory/apps.json` -> `hashtags.privacy_tags_configured` (the publish workflow refuses `submit_for_review` while it is empty).
