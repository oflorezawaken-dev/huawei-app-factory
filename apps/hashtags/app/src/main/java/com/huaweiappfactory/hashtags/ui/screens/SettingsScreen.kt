...
import com.huaweiappfactory.hashtags.BuildConfig
...
fun SettingsScreen(viewModel: SettingsViewModel, onBack: () -> Unit) {
    ...
    val openFullySelected by viewModel.openFullySelected.collectAsState()
    ...
    Column(modifier = Modifier.fillMaxSize().testTag("settings_screen")) {
        ...
        Column(
            ...
            Row(
                modifier = Modifier
                    .fillMaxWidth()
                    .clickable { viewModel.setOpenFullySelected(!openFullySelected) }
                    .padding(vertical = 12.dp)
                    .testTag("setting_open_selected"),
                verticalAlignment = Alignment.CenterVertically
            ) {
                Column(modifier = Modifier.weight(1f).padding(end = 12.dp)) {
                    Text(stringResource(R.string.settings_open_selected), style = MaterialTheme.typography.bodyLarge)
                    Text(
                        stringResource(R.string.settings_open_selected_desc),
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
                Switch(checked = openFullySelected, onCheckedChange = viewModel::setOpenFullySelected)
            }
        ...
        PetalBanner(placement = AdPlacement.SETTINGS_BANNER)
    }
